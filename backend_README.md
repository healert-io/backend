...
- **SQLite only**: No multi-replica support. One backend instance per deployment.
  PostgreSQL support planned for v0.3.0.
- **No Prometheus metrics**: Health observable via logs and `/health` only.
  Metrics endpoint planned for v0.3.0.
- **No authentication on read endpoints**: GET /friction and GET /events are
  public. Deploy behind Backstage proxy — do not expose directly to internet.
...
| Version | Features |
|---|---|
| v0.1.1 (current) | Exponential decay scoring, API key auth, rate limiting, WAL mode |
| v0.2.0 | Multi-namespace entity support, GitHub integration |
| v0.3.0 | PostgreSQL support, Prometheus metrics, exponential backoff |
...
| Repo | Description |
|---|---|
| [healert/agent](https://github.com/healert/agent) | Go agent + DaemonSet + healert.sh |
